...
Right: filter $m.applicationName == "payment"
...
Wrong: filter $m.applicationName == "Payment-Service"
Right: filter $m.applicationName == "payment-service"
```

**Cause 3: Missing Metadata Prefix**
```
Wrong: filter severity == ERROR
Right: filter $m.severity == ERROR

Wrong: filter applicationName == "web-app"
Right: filter $m.applicationName == "web-app"
```
...
Step 1: source logs
Step 2: source logs | filter $m.applicationName == "payment"
Step 3: source logs | filter $m.applicationName == "payment" | filter $m.severity == ERROR
```
...
If exact match fails:
  source logs
  | filter $m.applicationName ~~ "payment*"
